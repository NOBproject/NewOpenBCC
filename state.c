...
PRIVATE void dodowhile()
{
    ...
    if (sym == WHILESYM)
	nextsym();
	else if (sym == UNTILSYM)
	nextsym();
    else
	error("missing while or until at end of do");
...
PRIVATE void dountil()
{
    ...
    rparen();
    if (!isforever(testexp))
	jump(untilloop.contlab);	/* test at bottom */
    deflabel(wstatlab = getlabel());	/* back here if test succeeds */
    statement();
    deflabel(untilloop.contlab);
    jumpfalse(testexp, wstatlab);
    deflabel(untilloop.breaklab);
    deleteloop();
}
